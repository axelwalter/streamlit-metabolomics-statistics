...
try:
    ...
    if not st.session_state.data.empty:
        ...
        attribute = c1.selectbox(
            ...
            [
                ...
                if (
                    ...
                    and len(
                        set(st.session_state.md.loc[st.session_state.scaled.index, col])
                    )
                    > 1
                    and len(
                        set(st.session_state.md.loc[st.session_state.scaled.index, col])
                    )
                    != st.session_state.scaled.shape[0]
                )
            ],
        )
        matrix = c2.selectbox(
        ...
        permanova, pcoa_result = permanova_pcoa(
            st.session_state.scaled,
            matrix,
            st.session_state.md.loc[st.session_state.scaled.index, attribute],
        )
        ...
        if not permanova.empty:
            ...
            fig = get_pcoa_scatter_plot(
                pcoa_result,
                st.session_state.md.loc[st.session_state.scaled.index],
                attribute,
            )
