...
from src.visualizations import (
    get_anova_plot,
    get_tukey_volcano_plot,
    get_metabolite_boxplot,
)
...
try:
    ...
    if not st.session_state.data.empty:
                ...
                anova = pd.DataFrame(
                    np.fromiter(
                        gen_anova_data(
                            ...
                            st.session_state.scaled.columns,
                            attribute,
                        ),
                        dtype=dtypes,
                    )
        ...
        if not st.session_state.anova.empty:
            ...
            metabolite = st.selectbox(
                "Select metabolite",
                sorted(
                    list(
                        st.session_state.anova["metabolite"][
                            st.session_state.anova["significant"] == True
                        ]
                    )
                ),
            )
            fig = get_metabolite_boxplot(
                st.session_state.anova, st.session_state.data, metabolite, attribute
            )
        ...
        if c3.button("Run Tukey's") and e1 != e2:
            if not st.session_state.anova.empty:
                with st.spinner(
                    f"Running Tukey's post hoc test between {attribute} {e1} and {e2}..."
                ):
                    dtypes = [
                    ...
                    tukey = pd.DataFrame(
                        np.fromiter(
                            gen_pairwise_tukey(
                                st.session_state.data,
                                elements,
                                st.session_state.anova[
                                    st.session_state.anova["significant"]
                                ]["metabolite"],
                                attribute,
                            ),
                            dtype=dtypes,
