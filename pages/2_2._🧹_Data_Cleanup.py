...
if not st.session_state.ft.empty and not st.session_state.md.empty:
    ...
    sample_column = c2.selectbox(
        "Choose attribute for sample selection", st.session_state.md.columns
    )
    ...
    if blank_removal:
        non_samples_md = new_md.loc[
            [index for index in new_md.index if index not in samples.columns]
        ]
        c1, c2 = st.columns(2)
        table_title(
    ...
    normalization = st.checkbox(
        "Would you like to normalize values column-wise (sample centric)?", False
    )
    if normalization:
        if imputation:
            tmp_ft = imputed
...
"""
    ...
    if not st.session_state.data.empty:
        c1, c2 = st.columns(2)
        table_title(
            st.session_state.data, "Scaled feature table for statistical analysis", c1
        )
